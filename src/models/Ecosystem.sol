...
contract Ecosystem is EternalModel {
  struct Instance {
    ...
    address permissionModelAddress;
  ...
  function deserialize(address _daoAddress) external view returns (Instance memory record) {
    if (_exists(_daoAddress)) {
      ...
      record.permissionModelAddress = getAddress(
        keccak256(abi.encode(record.daoAddress, 'permissionModelAddress'))
      );
